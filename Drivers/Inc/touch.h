...
extern TP_Dev tp_dev;	 	//触屏控制器在touch.c里面定义
...
u8 tp_readXY(u16 *x,u16 *y);					//双方向读取(X+Y)
u8 tp_readXY2(u16 *x,u16 *y);					//带加强滤波的双方向坐标读取
void tp_drowTouchPoint(u16 x,u16 y,u16 color);//画一个坐标校准点
void TP_Draw_Big_Point(u16 x,u16 y,u16 color);	//画一个大点
void tp_saveAdjustData(void);						//保存校准参数
u8 tp_getAdjustData(void);						//读取校准参数
void tp_adjust(void);							//触摸屏校准
void tp_adjustInfo_show(u16 x0,u16 y0,u16 x1,u16 y1,u16 x2,u16 y2,u16 x3,u16 y3,u16 fac);//显示校准信息
//电阻屏/电容屏 共用函数
u8 tp_scan(u8 tp);								//扫描
u8 tp_init(void);								//初始化
